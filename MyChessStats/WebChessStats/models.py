...
class Game(models.Model):
    ...
    game_url = models.URLField(max_length=500, blank=True)  # Chess.com URL
    
    # Date and time
    date_played = models.DateField(db_index=True)  # From date field
    start_time = models.TimeField(null=True, blank=True)
    end_time = models.TimeField(null=True, blank=True)
    utc_date = models.DateField(null=True, blank=True)  # For Lichess
    utc_time = models.TimeField(null=True, blank=True)  # For Lichess
    
    # Game classification
    time_class = models.CharField(max_length=20, choices=TimeClass.choices, db_index=True)
    time_control = models.CharField(max_length=20, blank=True)  # e.g., "180+2", "600+0"
    ...
    termination = models.CharField(max_length=50, blank=True)  # From Lichess Termination field
    win_method = models.CharField(max_length=3, choices=WinMethod.choices, null=True, blank=True)
    outcome = models.CharField(max_length=50, blank=True)  # Chess.com outcome
    ...
    opponent_url = models.URLField(max_length=500, blank=True)  # Chess.com opponent URL
    rating_change = models.IntegerField(null=True, blank=True)  # +6, -6 format
    
    # Opening information
    eco = models.CharField(max_length=3, blank=True)  # ECO code
    opening = models.CharField(max_length=200, blank=True, db_index=True)
    opening_url = models.URLField(max_length=500, blank=True)  # Chess.com opening URL
    
    # Game data
    fen = models.CharField(max_length=100, blank=True)  # Final position FEN
    pgn = models.TextField(blank=True)  # Full PGN (essential for Lichess)
    moves = models.TextField(blank=True)  # Space-separated moves for easy parsing
    move_count = models.IntegerField(default=0)
    
    # Accuracy metrics (Chess.com specific)
    ...
    variant = models.CharField(max_length=50, blank=True)  # Standard, Chess960, etc.
    
    # Parsed move data (store as JSON for flexibility)
    moves_with_eval = models.JSONField(default=list, blank=True)  # Store moves with evaluations
    ...
    def __str__(self):
        return f"{self.platform} - {self.date_played} - {self.white_player} vs {self.black_player}"
    ...
    def get_win_method_display(self):
        """Get human-readable win method"""
        if self.win_method:
            return self.get_win_method_display()
        if self.termination:
    ...
    def parse_lichess_moves(self):
        """Parse Lichess PGN moves with evaluations into structured format"""
        if not self.moves_with_eval and self.pgn:
            # This would be implemented in import script
            pass
        return self.moves_with_eval
    
    def calculate_move_count_from_pgn(self):
        """Extract move count from PGN"""
        if self.pgn and not self.move_count:
            # Count moves by looking at move numbers
            # Simple implementation - count occurrences of move numbers
            import re
            moves_section = self.pgn.split('\n\n')[-1]  # Get moves after headers
            move_numbers = re.findall(r'\d+\.', moves_section)
            self.move_count = len(move_numbers)
        return self.move_count
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    is_active = models.BooleanField(default=True)  # Soft delete option
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=['my_rating']),
            models.Index(fields=['opponent_name']),
            models.Index(fields=['created_at']),  # For CRUD operations
        ]
